...
docker logs $COIN | grep $COIN-BTC | grep -v wrote  | awk '{print $1,$2}' | sed -e  's/\x1B\[\([0-9]\{1,2\}\(;[0-9]\{1,2\}\)\?\)\?[mGK]//g' | tail -1 > $START
